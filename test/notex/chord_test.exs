defmodule Notex.ChordTest do
  ...
  describe "built-in chord constructors" do
    test "builds a major seventh chord shape" do
      {:ok, chord} = build(major7())

      assert chord.voicings == [seven: [0], five: [0], three: [0], one: [0]]
    end
  end

  describe "built-in chord transformations" do
    test "adds a ninth to a chord shape" do
      {:ok, chord} =
        dominant7()
        |> add9()
        |> build()

      assert chord.voicings == [two: [1], flat_seven: [0], five: [0], three: [0], one: [0]]
    end
  end
